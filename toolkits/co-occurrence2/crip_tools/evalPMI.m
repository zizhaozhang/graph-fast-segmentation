...
%     pMarg(ii) = evaluate_batches(p2,F_unary(ii,:)',tol);
    pMarg(ii) = evaluate(p2,F_unary(ii,:)',tol);
...
function [v] = evaluate_batches(p,F,tol)
    ...
    end_i = 0;
    eps = 10e-6;
%     if size(F,2) > 1 % to prevent the bugs when evaluating pascal
    if m > 1
        for i=1:m
            start_i = (i-1)*n+1;
            end_i = start_i + n;
            end_i = min(end_i, size(F,2)); % small bug here, add this to fix
            tmp = F(:,start_i:end_i);
...
%         error('non-evaluable pmi')
        rad = rand(size(v,1),size(v,2));
        v = rad;
    end
   
%     if end_i < size(F,2) && size(F,2) > 1
%         tmp = F(:,end_i+1:end);
%         tmp = tmp+eps*(rand(size(tmp))-0.5);
%         v(end_i+1:end) = evaluate(p,tmp,tol);
%     else
