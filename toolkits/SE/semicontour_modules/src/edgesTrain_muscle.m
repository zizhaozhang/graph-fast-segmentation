...
dfs={'imWidth',32, 'gtWidth',16, 'nPos',5e5, 'nNeg',5e5, 'nImgs',inf, ...
  'nTrees',8, 'fracFtrs',1/2, 'minCount',1, 'minChild',8, ...
  'maxDepth',64, 'discretize','kmeans', 'nSamples',256, 'nClasses',2, ...
  'split','gini', 'nOrients',4, 'grdSmooth',0, 'chnSmooth',2, ...
  'simSmooth',8, 'normRad',4, 'shrink',1, 'nCells',5, 'rgbd',0, ...
  'stride',2, 'multiscale',1, 'sharpen',0, 'nTreesEval',4, ...
  'nThreads',4, 'nms',0, 'seed',1, 'useParfor',0, 'modelDir','models/', ...
  'modelFnm','model', 'bsdsDir','C:\Users\zizhaozhang\Desktop\MATLAB\Projects\cell_data\large_train_SRF'};
...
nChns = nChnsGrad+nChnsColor; opts.nChns = nChns; % here is how to change the dimension of features
...
%   B(:,[1:imRadius end-imRadius:end])=0;
  ...
  % prevent unlabled positive patch, for muscle
  pupp = 50;
  B([1:imRadius+pupp end-imRadius-pupp:end],:)=0;
  B(:,[1:imRadius+pupp end-imRadius-pupp:end])=0;
...
for i = 1:2
    idx = find(hs == i);
    idx = idx(1:max(1, length(idx)/100):end);
    figure(1), montage2(cell2array(segs_bk(idx)));
    d = getframe; d = d.cdata;
    imwrite(d,['./nodepatch/' num2str(i) '.bmp'],'bmp');
end
