function [E,O,inds,segs] = edgesDetect_retrain( I, model )
...
if( opts.multiscale )
  ...
  for i=1:k, s=ss(i); I1=imResample(I,s);
    if(nargout<4), [E1,~,inds{i}]=edgesDetect_retrain(I1,model);
    else [E1,~,inds{i},segs{i}]=edgesDetect_retrain(I1,model); end
  ...
  if opts.rgbd==2
      [chnsReg,chnsSim] = edgesChnsFillRgbd(I,opts); % for rgbd
  else
      [chnsReg,chnsSim] = edgesChns_retrain( I, opts );
  end
