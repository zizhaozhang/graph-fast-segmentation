function [baseRegions, nBaseRegions, Tree, V, pwStrength] = build_pylon2(ucm, opts)
...
% This is a faster version than pervious one used in the pylon model code.
% Zizhao @ UF
%%
...
% version 2 fast
neighbors8_s = sort(neighbors8, 1, 'ascend');
neighbors4_s = sort(neighbors4, 1, 'ascend');
[C8,~,ic8] = unique(neighbors8_s','rows');
[C4,~,ic4] = unique(neighbors4_s','rows');
for i = 1:max(size(C8,1), size(C4,1))
    if i <= size(C8,1)
        t = C8(i,:); t(t ==0 ) = [];
        bLength8(t,t) = bLength8(t,t) + length(find(ic8==i));
    end

    if i <= size(C4,1)
        t = C4(i,:); t(t==0) = [];
        bLength4(t,t) =  bLength8(t,t) + length(find(ic4==i));
    end
end
