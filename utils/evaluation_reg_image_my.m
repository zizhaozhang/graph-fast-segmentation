function [thresh, cntR, sumR, cntP, sumP, cntR_best] = evaluation_reg_image_my(imFile, inFile, gtFile, evFile2, evFile3, evFile4, nthresh)
...
for t = 1 : nthresh,
    ...
    if exist('segs', 'var')
        ...
        if size(ucm,1) ~= size(groundTruth{1}.Segmentation,1) % original version
            labels2 = bwlabel(ucm <= thresh(t),4);
            seg = labels2(2:2:end, 2:2:end);
            if min(seg(:)) == 0
                im = imread(imFile);
                seg = fillGap_sp(seg,im);
            end
        else % added to support the same size ucm 
            seg = bwlabel(ucm <= thresh(t),4);
